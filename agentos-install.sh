...
Options:
  ...
  --theme               Interface theme: auto|dark|light (default: saved value, else auto)
...
get_theme_state_file() {
  echo "$(get_config_home)/agentos-installer/theme"
}

load_theme_from_state() {
  if [[ "$THEME_EXPLICIT" == true ]]; then
    return
  fi

  local state_file
  state_file="$(get_theme_state_file)"
  if [[ ! -r "$state_file" ]]; then
    return
  fi

  local saved_theme=""
  IFS= read -r saved_theme < "$state_file" || true
  saved_theme="$(trim "$saved_theme")"
  if validate_theme "$saved_theme"; then
    THEME="$saved_theme"
    THEME_LOADED_FROM_STATE=true
  fi
}

persist_theme_if_needed() {
  if [[ "$THEME_PERSIST_REQUESTED" != true ]]; then
    return
  fi
  ...
  if ! mkdir -p "$state_dir" 2>/dev/null; then
    warn "Cannot create theme config directory at '$state_dir'"
    return
  fi
  if ! printf '%s\n' "$THEME" > "$state_file" 2>/dev/null; then
    warn "Cannot persist theme to '$state_file'"
    return
  fi
...
pick_theme_if_needed() {
  ...
  if [[ "$THEME_LOADED_FROM_STATE" == true ]]; then
    return
  fi
