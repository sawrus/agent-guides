...
if [[ "${1:-}" == "-C" ]]; then
  ...
  if [[ "${1:-}" == "pull" ]] && [[ "${2:-}" == "--ff-only" ]]; then
    ...
    if [[ -n "${AGENTIC_TEST_PULL_NEW_RULE:-}" ]]; then
      mkdir -p "$repo_dir/areas/software/backend/rules"
      printf '%s\n' "backend rule added by upgrade" > "$repo_dir/areas/software/backend/rules/$AGENTIC_TEST_PULL_NEW_RULE"
    fi
    exit 0
  fi
fi
...
assert_file_contains "$P1/opencode.json" "\"context7\""
assert_file_contains "$P1/.opencode/opencode.json" "\"_agentic\""
assert_file_contains "$P1/.opencode/opencode.json" "\"context7\""
...
echo "[e2e] Scenario 1a: Context7 project configs are generated for supported agent IDEs"
P1_CTX="$TMP_ROOT/project-context7"
HOME_CTX="$TMP_ROOT/home-context7"
HOME="$HOME_CTX" CONTEXT7_API_KEY="test-context7-key" "$CLI" install \
  --project-dir "$P1_CTX" \
  --agent-os claude,cursor,codex,gemini,opencode \
  --areas software \
  --specializations software.backend \
  --theme=light

assert_file_contains "$P1_CTX/.mcp.json" "\"context7\""
assert_file_contains "$P1_CTX/.mcp.json" "\"type\": \"http\""
assert_file_contains "$P1_CTX/.cursor/mcp.json" "\"context7\""
assert_file_contains "$P1_CTX/.gemini/settings.json" "\"httpUrl\": \"https://mcp.context7.com/mcp\""
assert_file_contains "$P1_CTX/.codex/config.toml" "[mcp_servers.context7]"
assert_file_contains "$P1_CTX/opencode.json" "\"type\": \"remote\""
assert_file_contains "$P1_CTX/opencode.json" "test-context7-key"
assert_file_contains "$P1_CTX/.agentic.json" "generated:context7-claude-config"
assert_file_contains "$P1_CTX/.agentic.json" "generated:context7-cursor-config"
assert_file_contains "$P1_CTX/.agentic.json" "generated:context7-gemini-config"
...
assert_file_contains "$OUT4" "No .agentic.json in current directory; knowledge base upgrade complete"

echo "[e2e] Scenario 4b: upgrade from managed project syncs new files without overwriting user edits"
P4="$TMP_ROOT/project-upgrade-sync"
OUT4B_INSTALL="$TMP_ROOT/upgrade-sync-install.log"
OUT4B="$TMP_ROOT/upgrade-sync.log"
HOME="$HOME_INSTALLED" PATH="$FAKE_GIT_BIN:/usr/bin:/bin" AGENTIC_TEST_GIT_LOG="$GIT_LOG" "$INSTALLED_BIN" install \
  --project-dir "$P4" \
  --agent-os default \
  --areas software \
  --specializations software.backend >"$OUT4B_INSTALL" 2>&1

P4_RULE="$P4/.agent/rules/backend.md"
assert_exists "$P4_RULE"
printf '%s\n' "# user edit before upgrade" >> "$P4_RULE"

(
  cd "$P4"
  HOME="$HOME_INSTALLED" PATH="$FAKE_GIT_BIN:/usr/bin:/bin" AGENTIC_TEST_GIT_LOG="$GIT_LOG" AGENTIC_TEST_PULL_NEW_RULE="backend-upgrade.md" "$INSTALLED_BIN" upgrade >"$OUT4B" 2>&1
)

assert_file_contains "$GIT_LOG" "git -C $HOME_INSTALLED/.local/share/agentic/repo pull --ff-only"
assert_file_contains "$OUT4B" "Detected managed project in $P4; syncing from upgraded knowledge base"
assert_file_contains "$OUT4B" "Skipping user-modified managed file: .agent/rules/backend.md"
assert_file_contains "$P4_RULE" "# user edit before upgrade"
assert_exists "$P4/.agent/rules/backend-upgrade.md"
assert_file_contains "$P4/.agentic.json" ".agent/rules/backend-upgrade.md"
